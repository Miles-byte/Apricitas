...
ICP_AUS_Graph <- ggplot() + #plotting US Crude Production
  ...
  xlab("Date") +
  scale_y_continuous(labels = scales::dollar_format(),limits = c(0,150),breaks = c(0,50,100,150,200), expand = c(0,0)) +
  ...
  scale_color_manual(name= NULL ,values = c("#FFE98F","#00A99D","#EE6055","#A7ACD9","#9A348E")) +
  annotation_custom(apricitas_logo_rast, xmin = as.Date("2016-01-01")-(.1861*(today()-as.Date("2016-01-01"))), xmax = as.Date("2016-01-01")-(0.049*(today()-as.Date("2016-01-01"))), ymin = 0-(.3*150), ymax = 0) + #these repeated sections place the logo in the bottom-right of each graph. The first number in all equations is the chart's origin point, and the second number is the exact length of the x or y axis
  coord_cartesian(clip = "off")
...
EDUCATION_EXPORTS <- download_abs_data_cube("international-trade-supplementary-information-financial-year","536805500309") %>%
  ...
  drop_na() %>%
  mutate(date = seq.Date(from = as.Date("2001-06-01"), to = as.Date("2024-06-01"), by = "year"))
...
Consumption <- read_abs(series_id = "A2303954C") %>%
  subset(date >= as.Date("2022-01-01")) %>%
  select(date, value) %>%
  mutate(name = "Consumption")

Investment <- read_abs(series_id = "A2303992T") %>%
  subset(date >= as.Date("2022-01-01")) %>%
  select(date, value) %>%
  mutate(name = "Investment")

Inventories <- read_abs(series_id = "A2304020T") %>%
  subset(date >= as.Date("2022-01-01")) %>%
  select(date, value) %>%
  mutate(name = "Inventories")

Net_Exports <- merge(read_abs(series_id = "A2304024A"),read_abs(series_id = "A2304026F"), by = "date") %>%
  subset(date >= as.Date("2022-01-01")) %>%
  transmute(date, value = value.x+value.y) %>%
  mutate(name = "Net Exports")

Government <- merge(read_abs(series_id = "A2304008A"),read_abs(series_id = "A2303952X"), by = "date") %>%
  subset(date >= as.Date("2022-01-01")) %>%
  transmute(date, value = value.x+value.y) %>%
  mutate(name = "Government")

AUS_GDP_CONTRIB <- rbind(Consumption,Investment,Inventories,Net_Exports,Government) %>%
  mutate(date = date %m-% months(2))

AUS_RGDPQUARTERLY <- read_abs(series_id = "A2304370T") %>%
  subset(date >= as.Date("2022-01-01")) %>%
  mutate(date = date %m-% months(2))

GDPContribAUS_Graph <- ggplot(AUS_GDP_CONTRIB, aes(fill=name, x=date, y=value/100)) +
  geom_bar(position="stack", stat="identity", size = 0, color = NA) + #putting color to NA gets rid of borders
  annotate("hline", y = 0, yintercept = 0, color = "white", size = 0.5) +
  geom_point(data = AUS_RGDPQUARTERLY, aes(x=date, y = value/100), size = 3, fill ="black", color = "white", shape = 23) +
  guides(fill = guide_legend(override.aes = list(shape = NA)), color = "none") +
  xlab("Date") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),limits = c(-0.02,0.03), breaks = c(-0.02,-0.01,0,0.01,0.02,0.03), expand = c(0,0)) +
  ylab("Contributions, Percent, Seasonally Adjusted at Quarterly Rates") +
  ggtitle("Contributions to Australian Real GDP Growth") +
  labs(caption = "Graph created by @JosephPolitano using Australian Bureau of Statistics data",subtitle = NULL) +
...
ggsave(dpi = "retina",plot = GDPContribAUS_Graph, "GDP Contributions Australia.png", type = "cairo-png", width = 9.02, height = 5.76, units = "in") #cairo gets rid of anti aliasing
