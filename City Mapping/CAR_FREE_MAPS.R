...
VA_CAR_FREE <- acs.fetch(geography = VA,endyear = 2024,table.number="B08201")
...
MD_CAR_FREE <- acs.fetch(geography = MD,endyear = 2024,table.number="B08201")
...
DC_CAR_FREE <- acs.fetch(geography = DC,endyear = 2024,table.number="B08201")
...
DC_SHAPE <- tracts(state="DC",county= c(001), year = 2024)
...
DC_CAR_FREE_MAP <- ggplot() + 
  ...
  labs(caption = "Graph created by @JosephPolitano using ACS 2024 5-Yr Estimates") +
  theme_apricitas + 
...
IL_CAR_FREE <- acs.fetch(geography = IL,endyear = 2024,table.number="B08201")
...
IN_CAR_FREE <- acs.fetch(geography = IN,endyear = 2024,table.number="B08201")
...
MA_CAR_FREE <- acs.fetch(geography = MA,endyear = 2024,table.number="B08201")
...
PA_CAR_FREE <- acs.fetch(geography = PA,endyear = 2024,table.number="B08201")
...
NJ_CAR_FREE <- acs.fetch(geography = NJ,endyear = 2024,table.number="B08201")
...
CA_CAR_FREE <- acs.fetch(geography = CA,endyear = 2024,table.number="B08201")
