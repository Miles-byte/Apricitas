...
Childcare$date <- seq(as.Date("2018-01-01"), as.Date("2022-01-01"), "months")
...
OwnIllnessNoWork$date <- seq(as.Date("2018-01-01"), as.Date("2022-01-01"), "months")
...
OwnIllnessPartTime$date <- seq(as.Date("2018-01-01"), as.Date("2022-01-01"), "months")

PandemicLostWork <- data.frame(date = seq(as.Date("2020-05-01"), as.Date("2022-01-01"), "months"), value = c(48839,40368,31281,24225,19385,15070,14805,15819,14755,13348,11391,9378,7907,6209,5150,5647,5032,3830,3640,3101,6043))
...
EPOP55Plus$date <- seq(as.Date("2018-01-01"), as.Date("2022-01-01"), "months")
...
UnpaidAbsences$date <- seq(as.Date("2018-01-01"), as.Date("2022-01-01"), "months")
...
PandemicLostWork_Graph <- ggplot() + #plotting weekly initial claims for 2014-2019
  ...
  scale_x_date(limits = c(as.Date("2021-01-01"),as.Date("2022-01-01"))) +
  ggtitle("Omicron and the Labor Market") +
...
TRNSPT_Graph <- ggplot() + #plotting transportation and warehousing
  ...
  xlab("Date") +
  ylab("Millions of Employees") +
  scale_y_continuous(labels = scales::number_format(suffix = "M", accuracy = 0.5), breaks = c(5,5.5,6), limits = c(5,6.35), expand = c(0,0)) +
  ...
  theme_apricitas + theme(legend.position = c(.55,.92)) +
  scale_color_manual(name= NULL,values = c("#FFE98F","#00A99D","#EE6055","#A7ACD9")) +
  annotation_custom(apricitas_logo_rast, xmin = as.Date("2019-01-01")-(.1861*1011), xmax = as.Date("2019-01-01")-(0.049*1011), ymin = 5-(.3*1.35), ymax = 5) +
  coord_cartesian(clip = "off")
