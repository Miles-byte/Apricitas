...
GLI_BLS_YOY <- bls_api("CES0500000017", startyear = 2017, endyear = 2023, calculations = TRUE, Sys.getenv("BLS_KEY")) %>% #headline cpi data
  ...
  .[nrow(.):1,] %>%
  mutate(date =(seq(as.Date("2017-01-01"), as.Date("2023-01-01"), by = "month"))) %>%
  select(-latest) %>%
  drop_na()
...
GLI_GROWTH_graph <- ggplot() + #plotting Wage Growth
  ...
  labs(caption = "Graph created by @JosephPolitano using BLS and BEA Data",subtitle = "Gross Labor Income Picked Up In January Payrolls Data") +
