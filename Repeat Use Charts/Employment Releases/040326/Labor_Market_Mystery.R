...
EMPLOYMENT_Revisions_Graph <- ggplot() + #CPS with NFP adjusted concepts
  geom_line(data = PAYEMS_OLD, aes(x=date, y = value/1000, color = "Nonfarm Payrolls—Unrevised"), size = 1.25) + 
  ...
  ggtitle("Solving a Labor Market Mystery") +
  ...
  theme_apricitas + theme(legend.position = c(.375,.87)) +
  theme(legend.key.width =  unit(.81, "cm")) +
  scale_color_manual(name= NULL,values = c("#EE6055","#FFE98F","#00A99D"), breaks = c("Nonfarm Payrolls—Unrevised","Nonfarm Payrolls—Revised","Household Survey Adjusted to Nonfarm Payrolls Concepts")) +
