...
BOS <- bls_api("SMS25716500000000001", startyear = 2020, registrationKey = Sys.getenv("BLS_KEY")) %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  arrange(date) %>%
  mutate(pct_growth_2020 = (value-value[1])/value[1],
         raw_growth_2020 = (value-value[1]),
...
CLE <- bls_api("SMS39174600000000001", startyear = 2020, registrationKey = Sys.getenv("BLS_KEY")) %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  arrange(date) %>%
  mutate(pct_growth_2020 = (value-value[1])/value[1],
         raw_growth_2020 = (value-value[1]),
...
PRO <- bls_api("SMS44772000000000001", startyear = 2020, registrationKey = Sys.getenv("BLS_KEY")) %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  arrange(date) %>%
  mutate(pct_growth_2020 = (value-value[1])/value[1],
         raw_growth_2020 = (value-value[1]),
...
BOS_NSA <- bls_api("SMU25716500000000001", startyear = 2020, registrationKey = Sys.getenv("BLS_KEY")) %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  arrange(date) %>%
  mutate(pct_growth_2020 = (value-value[1])/value[1],
         raw_growth_2020 = (value-value[1]),
...
CLE_NSA <- bls_api("SMU39174600000000001", startyear = 2020, registrationKey = Sys.getenv("BLS_KEY")) %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  arrange(date) %>%
  mutate(pct_growth_2020 = (value-value[1])/value[1],
         raw_growth_2020 = (value-value[1]),
...
PRO_NSA <- bls_api("SMU44772000000000001", startyear = 2020, registrationKey = Sys.getenv("BLS_KEY")) %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  arrange(date) %>%
  mutate(pct_growth_2020 = (value-value[1])/value[1],
         raw_growth_2020 = (value-value[1]),
...
BLS_NFP_MSA_BUB_YOY_GRAPH <- MSA_map_US_NSA %>%
  ...
  geom_sf(data = filter(states, !state_abbv %in% c("HI", "AK", "AS", "GU", "MP", "VI")), color = "grey20", fill = "grey50", lwd = 0.25) + # Black borders for states
  geom_sf(fill = "grey75") +
  geom_point(data = MSA_map_US_centroids, aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], fill = raw_growth_yoy > 0, size = raw_growth_yoy/1000), shape = 21, alpha = 0.5, show.legend = TRUE) +
