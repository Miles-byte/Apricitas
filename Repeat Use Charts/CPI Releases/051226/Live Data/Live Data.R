...
CPIPCT <- bls_api("CUUR0000SA0", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
  subset(date >= as.Date("2019-01-01")) %>% #cpi pct data
  select(-any_of("latest")) %>%
  drop_na()

CPILFEPCT <- bls_api("CUUR0000SA0L1E", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12)) %>%
...
CPIPCT_MOM <- bls_api("CUSR0000SA0", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
...
CPILFEPCT_MOM <- bls_api("CUSR0000SA0L1E", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,1))/lead(value,1)) %>%
  subset(date >= as.Date("2019-01-01")) %>%
  select(-any_of("latest")) %>%
...
latest_two <- function(df) df %>% arrange(desc(date)) %>% slice(1:2) %>% pull(value)

yoy_phrase <- function(cur, prev) {
  verb <- ifelse(cur > prev, "rose to", ifelse(cur < prev, "fell to", "was unchanged at"))
  sprintf("%s %s%% year-on-year", verb, formatC(cur*100, format = "f", digits = 1))
}

mom_phrase <- function(cur) {
  if (cur > 0) sprintf("increasing %s%% month-on-month", formatC(cur*100, format = "f", digits = 1))
  else if (cur < 0) sprintf("decreasing %s%% month-on-month", formatC(abs(cur)*100, format = "f", digits = 1))
  else "was unchanged month-on-month"
}

cpi_y  <- latest_two(CPIPCT);        cpi_m  <- latest_two(CPIPCT_MOM)
core_y <- latest_two(CPILFEPCT);     core_m <- latest_two(CPILFEPCT_MOM)

sprintf(
  "NEW: CPI inflation %s, %s\n\nCore CPI inflation %s, %s",
  yoy_phrase(cpi_y[1], cpi_y[2]),
  mom_phrase(cpi_m[1]),
  yoy_phrase(core_y[1], core_y[2]),
  mom_phrase(core_m[1])
)
...
CPIPCT_COMMODITIES <- bls_api("CUUR0000SAC", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
  subset(date >= as.Date("2019-01-01")) %>%#cpi rent data
  select(-any_of("latest")) %>%
...
CPIPCT_DURABLES <- bls_api("CUUR0000SAD", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
  subset(date >= as.Date("2019-01-01")) %>%#cpi rent data
  select(-any_of("latest")) %>%
...
CPIPCT_NONDURABLES <- bls_api("CUUR0000SAN", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
  subset(date >= as.Date("2019-01-01")) %>%#cpi rent data
  select(-any_of("latest")) %>%
...
CPIPCT_GOODSEXFE <- bls_api("CUUR0000SACL1E", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
  subset(date >= as.Date("2019-01-01")) %>%#cpi rent data
  select(-any_of("latest")) %>%
...
CPIPCT_GOODS_Graph <- ggplot() + #plotting CPI/PCEPI against 2% CPI trend
  ...
  geom_line(data=CPIPCT_DURABLES, aes(x=date,y= value,color= "Durables"), size = 1.25) +
  geom_line(data=CPIPCT_NONDURABLES, aes(x=date,y= value,color= "Nondurables"), size = 1.25) +
...
COFFEE_PRICES <- bls_api("CUUR0000SEFP01", startyear = 2016, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
  subset(date >= as.Date("2019-01-01")) %>%#cpi rent data
  select(-any_of("latest")) %>%
  drop_na() #cpi rent data
...
COFFEE_PRICES_INDEX <- bls_api("CUSR0000SEFP01", startyear = 2019, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
...
CPI_COFFEE_INDEX_Graph <- ggplot() + #plotting CPI/PCEPI against 2% CPI trend
  ...
  ggtitle("US Coffee Prices, Growth Since 2020") +
  labs(caption = "Graph created by @JosephPolitano using BLS data",subtitle = paste0("Coffee Prices are Up ",round(COFFEE_PRICES_INDEX$value[nrow(COFFEE_PRICES_INDEX)]*100),"% Since January 2020")) +
...
BANANA_PRICES <- bls_api("CUUR0000SEFK02", startyear = 2016, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
  subset(date >= as.Date("2019-01-01")) %>%
  select(-any_of("latest")) %>%
...
ELECTRICITY_PRICES <- bls_api("CUUR0000SEHF01", startyear = 2016, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
  subset(date >= as.Date("2019-01-01")) %>%
  select(-any_of("latest")) %>%
...
ELECTRICITY_PRICES_INDEX <- bls_api("CUSR0000SEHF01", startyear = 2016, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  subset(date >= as.Date("2019-01-01")) %>%
  arrange(rev(desc((date)))) %>%
...
CPI_ELECTRICITY_INDEX_Graph <- ggplot() + #plotting CPI/PCEPI against 2% CPI trend
  ...
  # annotate("text",label = "Inflation Reduction Act Signed", x= as.Date("2021-09-01"), y = 0.0075, color = "white", size = 5.5) +
  xlab("Date") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = c(-.10,0.45), breaks = c(-.1,0,0.1,0.2,0.3,0.4), expand = c(0,0)) +
  ylab("Percent Change Since Jan 2020") +
  ...
  theme_apricitas + theme(legend.position = c(.20,.50)) +
  scale_color_manual(name= NULL,values = c("#FFE98F","#00A99D","#EE6055","#A7ACD9","#9A348E")) +
  annotation_custom(apricitas_logo_rast, xmin = as.Date("2019-01-01")-(.1861*(today()-as.Date("2019-01-01"))), xmax = as.Date("2019-01-01")-(0.049*(today()-as.Date("2019-01-01"))), ymin = -.10-(.3*0.55), ymax = -0.10) +
  coord_cartesian(clip = "off")
...
USED_CAR_PRICES <- bls_api("CUUR0000SETA02", startyear = 2016, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
  subset(date >= as.Date("2019-01-01")) %>%
  select(-any_of("latest")) %>%
...
AUDIO_EQUIPMENT_PRICES <- bls_api("CUUR0000SERA05", startyear = 2016, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
  subset(date >= as.Date("2019-01-01")) %>%
  select(-any_of("latest")) %>%
...
APPLIANCE_PRICES <- bls_api("CUUR0000SEHK", startyear = 2016, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
...
CPIRENT <- bls_api("CUSR0000SEHA", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  select(-any_of("latest")) %>%
  drop_na() %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12))  %>%
  subset(date >= as.Date("2019-01-01")) #cpi rent data
CPIORENT <- bls_api("CUSR0000SEHC", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  select(-any_of("latest")) %>%
  drop_na() %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,12))/lead(value,12)) %>%
...
CPIUSEDCARS <- bls_api("CUSR0000SETA02", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  select(-any_of("latest")) %>%
  drop_na() %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  subset(date >= as.Date("2019-01-01"))
...
CPIUSEDCARS_GROWTH <- bls_api("CUUR0000SETA02", startyear = 2017, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  select(-any_of("latest")) %>%
  drop_na() %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  arrange(date) %>%
...
CPIRENTmonth <- bls_api("CUSR0000SEHA", startyear = 2018, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  select(-any_of("latest")) %>%
  drop_na() %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,1))/lead(value,1)) %>%
...
CPIOERmonth <- bls_api("CUSR0000SEHC", startyear = 2018, endyear = 2026, calculations = TRUE, Sys.getenv("BLS_KEY"))%>% #headline cpi data
  select(-any_of("latest")) %>%
  drop_na() %>%
  mutate(date = as.Date(as.yearmon(paste(periodName, year), "%b %Y"))) %>%
  mutate(value = (value-lead(value,1))/lead(value,1)) %>%
