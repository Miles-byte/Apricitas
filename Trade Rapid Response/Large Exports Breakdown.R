...
#Backup option because other downloads were not working




US_COUNTRIES_HS4_EXPORTS_CHINA <- getCensus(
  key = Sys.getenv("CENSUS_KEY"),
  name = "timeseries/intltrade/exports/hs",
  vars = c("ALL_VAL_YR","E_COMMODITY","E_COMMODITY_LDESC","CTY_CODE","COMM_LVL"),
  time = "2024-12",
  COMM_LVL = "HS4",
  #E_COMMODITY = "8542",
  CTY_CODE = "5700"
...
TOP_EXPORT_LEVELS_TOTAL <- US_COUNTRIES_HS4_EXPORTS_CHINA %>%
  mutate(ALL_VAL_YR = as.numeric(ALL_VAL_YR)) %>%
  group_by(E_COMMODITY) %>%
  select(ALL_VAL_YR,E_COMMODITY,E_COMMODITY_LDESC) %>%
  mutate(ALL_VAL_YR = sum(ALL_VAL_YR, na.rm = TRUE)) %>%
  ...
  ungroup() %>%
  arrange(desc(ALL_VAL_YR)) %>%
  filter(E_COMMODITY != "9880")


TOP_EXPORT_SHARES_TOTAL <- US_COUNTRIES_HS4_EXPORTS_TOTAL %>%
  mutate(ALL_VAL_YR = as.numeric(ALL_VAL_YR)) %>%
  group_by(E_COMMODITY) %>%
  select(ALL_VAL_YR,E_COMMODITY,E_COMMODITY_LDESC) %>%
  mutate(ALL_VAL_YR = sum(ALL_VAL_YR, na.rm = TRUE)) %>%
  unique() %>%
  ungroup() %>%
  merge(.,US_COUNTRIES_HS4_EXPORTS_CHINA,by = "E_COMMODITY") %>%
  transmute(E_COMMODITY, E_COMMODITY_LDESC = E_COMMODITY_LDESC.x,ALL_VAL_YR = as.numeric(ALL_VAL_YR.y),ALL_VAL_SHARE = as.numeric(ALL_VAL_YR.y)/ALL_VAL_YR.x) %>%
  filter(ALL_VAL_YR >= 1000000000) %>%
  filter(E_COMMODITY != "2716") %>% #excluding electricity
  arrange(desc(ALL_VAL_SHARE))
...
TOP_EXPORT_LEVELS_TOTAL_TOP5 <- TOP_EXPORT_LEVELS_TOTAL %>%
  slice(1:5) %>%
  mutate(I_COMMODITY_LDESC = c("Soybeans","Aircraft & Parts","Semiconductors","Vaccines/Antibodies/Blood","Petroleum Gases")) %>%
...
TOP_EXPORT_SHARES_TOTAL_TOP5 <- TOP_EXPORT_SHARES_TOTAL %>%
  slice(1:5) %>%
  mutate(I_COMMODITY_LDESC = c("Sorghum","Soybeans","Copper Scrap","Cotton","Frozen Beef")) %>%
  mutate(I_COMMODITY_LDESC = factor(I_COMMODITY_LDESC, levels = rev(I_COMMODITY_LDESC)))
...
Large_Exports <- ggarrange(plot_0,Large_Exports, nrow = 2, heights = c(4,20), widths = 10) %>%
  annotate_figure(.,bottom = text_grob("\nGraph Created by @Josephpolitano Using Census Data\nNOTE: Breakdowns Based on 4-Digit HS Codes. % of Imports Data Only For Goods Above $1B in Exports", color = "grey55",hjust = 1, x = 1, size = 10))+ bgcolor("#252A32") + border("#252A32")
