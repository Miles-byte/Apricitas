...
JUNE_HS4_IMPORTS_NO_TARIFF <- JUNE_HS4_IMPORTS_NO_TARIFF %>%
  ...
  mutate(Category = factor(Category, levels = rev(c("Computers & Parts","Pharmaceuticals","Mexico (USMCA-compliant)","Canada (USMCA-compliant)","Energy","Precious Metals","Smartphones","Critical Minerals","Copper & Related","Lumber & Related","Semiconductor Equipment"))))
...
LARGEST_TARIFF_EXEMPTIONS <- ggplot(data = JUNE_HS4_IMPORTS_NO_TARIFF, aes(x = Category, y = CON_VAL_MO/1000000000, fill = Category == "Computers & Parts")) +
  ...
  xlab(NULL) +
  ggtitle("Largest Tariff Exemptions, September 2025") +
  ylab("US Imports in Category, September 2025") +
