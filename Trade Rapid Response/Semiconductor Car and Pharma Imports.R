...
CAR_IMPORTS <- CAR_IMPORTS_BULK %>%
  ...
  # filter(time == "2024-12") %>%
  # filter(!CTY_NAME %in% c("TOTAL FOR ALL COUNTRIES", "OECD", "APEC", "NATO","USMCA (NAFTA)","NAFTA","NORTH AMERICA", "TWENTY LATIN AMERICAN REPUBLICS","LAFTA","EUROPE","ASIA","PACIFIC RIM COUNTRIES","SOUTH AMERICA","EURO AREA","ASEAN","CENTRAL AMERICA","CAFTA-DR","CACM")) %>%
  # group_by(CTY_NAME) %>%
