...
AGG_RECIPROCAL_TARIFF_RATE_CAPS <- merge(US_IMPORTS_BULK,US_EXPORTS_BULK, by = "CTY_NAME") %>%
...
AGG_TARIFF_BY_CATEGORY <- AGG_TARIFF_ANALYSIS %>%
...
AGG_TARIFF_BY_COUNTRY <- AGG_TARIFF_ANALYSIS %>%
...
TARIFF_BY_COUNTRY_RECIPROCAL_GRAPH <- ggplot(data = TARIFF_BY_COUNTRY, aes(x = tariff, y = value/1000000000, fill = region)) +
  ...
  annotate(geom = "segment", x = .34, xend = .315, y = 353, yend = 373, color = "white", lwd = .75) +
  annotate(geom = "text", label = "China", x = .29, y = 373, color ="white",size = 4, lineheight = unit(0.85,"cm")) +
  ...
  labs(caption = "Graph created by @JosephPolitano using Census Data.", subtitle = "Trump's April 2nd Tariff Hike is a 27% Tax on Affected Imports and 13% on the Whole Import Mix") +
