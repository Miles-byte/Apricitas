...
CN_MX_CA_EXPORTS_GRAPH <- ggplot() + #plotting integrated circuits exports
  ...
  xlab("Date") +
  scale_y_continuous(labels = scales::dollar_format(suffix = "B", accuracy = 1),limits = c(-70,55),breaks = c(-50,-25,0,25,50), expand = c(0,0)) +
  ylab("Dollars, Not Seasonally Adjusted Annual Rate") +
  ...
  annotation_custom(apricitas_logo_rast, xmin = as.Date("2024-01-01")-(.1861*(today()-as.Date("2024-01-01"))), xmax = as.Date("2024-01-01")-(0.049*(today()-as.Date("2024-01-01"))), ymin = -70-(.3*(125)), ymax = -60) +
  coord_cartesian(clip = "off") + theme(strip.text = element_text(size = 15, color = "white", face = "bold"))
