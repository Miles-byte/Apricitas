...
TARIFF_PCT_GDP_Graph <- ggplot() + #plotting rent by A/B/C City Size
  ...
  xlab("Date") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1),limits = c(0,0.0125), expand = c(0,0)) +
  ylab("Percent") +
  ...
  annotation_custom(apricitas_logo_rast, xmin = as.Date("1929-01-01")-(.1861*(today()-as.Date("1929-01-01"))), xmax = as.Date("1929-01-01")-(0.049*(today()-as.Date("1929-01-01"))), ymin = 0-(.3*.0125), ymax = 0) + #these repeated sections place the logo in the bottom-right of each graph. The first number in all equations is the chart's origin point, and the second number is the exact length of the x or y axis
