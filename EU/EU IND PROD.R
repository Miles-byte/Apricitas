...
EU_IND_PROD_BULK <- get_eurostat("sts_inpr_m",legacy_bulk_download = FALSE, cache = FALSE)

EU_IND_PROD_MAP <- EU_IND_PROD_BULK %>%
  subset(unit == "PCH_SM" & nace_r2 == "B-D") %>%
...
EU_IND_PROD_SHP <- ne_countries(scale = "medium", returnclass = "sf") %>%
...
EU_IND_PROD_SHP <- full_join(EU_IND_PROD_SHP, EU_IND_PROD_MAP,by = "geo") %>%
  select(geometry, values, geo, name, time) %>%
  mutate(label = values) %>%
  st_as_sf()

EU_IND_PROD_CENTROIDS <- ne_countries(scale = "medium", returnclass = "sf") %>%
  subset(., continent == "Europe" | sovereignt %in% c("Turkey","Cyprus","Malta")) %>%
  mutate(iso_a2 = ifelse(sovereignt == "Kosovo", "XK", iso_a2)) %>%
  st_transform(., crs = 3035) %>%
  st_as_sf() %>% 
  ...
  as.data.frame() %>% 
  rename(long = X, lat = Y) %>% 
  bind_cols(EU_IND_PROD_SHP %>% st_drop_geometry(), .) %>%
...
EU_IND_PROD_GRAPH <- ggplot(data = EU_IND_PROD_SHP, aes(fill = values)) +
  geom_sf(color = NA) +
  ...
  ggtitle(paste0("EU Industrial Production Growth,YoY, ",
                 month(EU_IND_PROD_SHP$time[36], label = TRUE, abbr = TRUE),
                 " ", year(EU_IND_PROD_SHP$time[36])))+
  ...
  labs(fill = NULL) +
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("SI")), 
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("HR")), 
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("DK")), 
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("NL")), 
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("BE")), 
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("LU")), 
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("EE")), 
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("LV")), 
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("LT")), 
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("CY")), 
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("GR")), 
    aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label, 1)), "%")), 
    size = 3, 
    color = "black",
    hjust = 0.5,
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("MT")), 
    aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label, 1)), "%")), 
    size = 3, 
    color = "black",
    hjust = 0.5,
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("PT")), 
    aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label, 1)), "%")), 
    size = 3, 
    color = "black",
    hjust = 0.5,
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("IE")), 
    aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label, 1)), "%")), 
    size = 3, 
    color = "black",
    hjust = 0.5,
  ...
  geom_label(
    data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("SK")), 
    aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label, 1)), "%")), 
    size = 3, 
    color = "black",
    hjust = 0.5,
  ...
  geom_text(data = filter(EU_IND_PROD_CENTROIDS, !geo %in% c("AD","IS","UA","RS","TR","MD","AX","CH","SM","VA","ME","AL","MK","BA","JE","IM","FO","GB","BB","LI","MC","GG","XK","NO","IE","LU","NE","EE","LT","NL","BE","DK","BY","LV","MT","GR","CY","SI","SK","HR","BG","CZ","HU","AT","PT")), aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label, 1)), "%")), size = 3, color = "black", check_overlap = TRUE,fontface = "bold",lineheight = 0.75) +
  geom_text(data = filter(EU_IND_PROD_CENTROIDS, geo %in% c("BG","CZ","HU","AT")), aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label, 1)), "%")), size = 2.5, color = "black", check_overlap = TRUE,fontface = "bold",lineheight = 0.75) +
  theme(plot.title.position = "panel") + theme(plot.title = element_text(size = 26, hjust = 0, margin = margin(l = -20))) +
...
ggsave(dpi = "retina",plot = EU_IND_PROD_GRAPH, "EU IND PROD Graph.png", type = "cairo-png", width = 9.02, height = 5.76, units = "in") #cairo gets rid of anti aliasing




EU_IND_PROD_LINE <- EU_IND_PROD_BULK %>%
  subset(unit == "I21" & s_adj == "SCA" & nace_r2 == "B-D") %>%
  transmute(geo, time = TIME_PERIOD, values) %>%
...
EU_IND_PROD_LINE_GRAPH <- ggplot() + 
  annotate(geom = "hline",y = 0.0,yintercept = 0.0, size = .25,color = "white") +
  geom_line(data=EU_IND_PROD_LINE, aes(x=time,y= EA20,color="Euro Area"), size = 1.25) +
  geom_line(data=EU_IND_PROD_LINE, aes(x=time,y= EU27_2020,color="European Union"), size = 1.25) +
  xlab(NULL) +
  scale_y_continuous(labels = scales::number_format(accuracy = 1), expand = c(0,0), limits = c(70,110)) +
  ylab(NULL) +
  ggtitle(NULL) +
...
EU_IND_PROD_MAP_SMALL <- ggplot(data = EU_IND_PROD_SHP, aes(fill = values)) +
  geom_sf(color = NA) +
  geom_sf(color = "black", fill = NA, lwd = 0.35) + # Black borders for states
  scale_fill_gradientn(colors = c("#EE6055","#F5B041","#FFE98F", "#AED581","#00A99D","#3083DC"), label = scales::number_format(suffix = "%", accuracy = 1), expand = c(0,0)) +
  ggtitle("Year-on-Year Growth") +
  # ggtitle(paste0(" EU Unemployment Rate, %: ",
  #                month(EU_IND_PROD_SHP$time[10], label = TRUE, abbr = TRUE),
  #                " ", year(EU_IND_PROD_SHP$time[10])))+
...
tgrob <- text_grob("EU Industrial Production",face = "bold",size = 29,color = "white")
...
EU_IND_PROD_GGARRANGE <- ggarrange(EU_IND_PROD_LINE_GRAPH,EU_IND_PROD_MAP_SMALL, ncol = 2, nrow = 1, heights = c(5,20), widths = 10, common.legend = FALSE) + bgcolor("#252A32") + border("#252A32")

EU_IND_PROD_GGARRANGE <- ggarrange(plot_0,EU_IND_PROD_GGARRANGE, nrow = 2, heights = c(4,20), widths = 10) %>%
...
ggsave(dpi = "retina",plot = EU_IND_PROD_GGARRANGE, "EU IND PROD GGARANGE Graph.png", type = "cairo-png", width = 9.02, height = 5.76, units = "in")
