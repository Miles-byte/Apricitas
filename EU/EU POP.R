...
EU_POP_SHP <- ne_countries(scale = "medium", returnclass = "sf") %>%
...
EU_POP_SHP <- full_join(EU_POP_SHP, EU_POP,by = "geo") %>%
  select(geometry, values, geo, name, time) %>%
  mutate(label = values) %>%
  st_as_sf()

EU_POP_CENTROIDS <- ne_countries(scale = "medium", returnclass = "sf") %>%
  subset(., continent == "Europe" | sovereignt %in% c("Turkey","Cyprus","Malta")) %>%
  mutate(iso_a2 = ifelse(sovereignt == "Kosovo", "XK", iso_a2)) %>%
  st_transform(., crs = 3035) %>%
  st_as_sf() %>% 
  ...
  as.data.frame() %>% 
  rename(long = X, lat = Y) %>% 
  bind_cols(EU_POP_SHP %>% st_drop_geometry(), .) %>%
...
EU_POP_GRAPH <- ggplot(data = EU_POP_SHP, aes(fill = values)) +
  geom_sf(color = NA) +
  ...
  ggtitle(paste0("  EU Population Growth, YoY %: ", "Q", quarter(EU_POP_SHP$time[10]), " ", year(EU_POP_SHP$time[10]))) +
  ...
  labs(fill = NULL) +
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("SI")), 
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("HR")), 
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("DK")), 
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("NL")), 
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("BE")), 
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("LU")), 
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("EE")), 
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("LV")), 
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("LT")), 
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("CY")), 
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("GR")), 
    aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label *100, 1)), "%")), 
    size = 3, 
    color = "black",
    hjust = 0.5,
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("MT")), 
    aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label *100, 1)), "%")), 
    size = 3, 
    color = "black",
    hjust = 0.5,
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("PT")), 
    aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label *100, 1)), "%")), 
    size = 3, 
    color = "black",
    hjust = 0.5,
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("IE")), 
    aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label *100, 1)), "%")), 
    size = 3, 
    color = "black",
    hjust = 0.5,
  ...
  geom_label(
    data = filter(EU_POP_CENTROIDS, geo %in% c("SK")), 
    aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label *100 * 100, 1)), "%")), 
    size = 3, 
    color = "black",
    hjust = 0.5,
  ...
  geom_text(data = filter(EU_POP_CENTROIDS, !geo %in% c("AD","IS","UA","RS","TR","MD","AX","CH","SM","VA","ME","AL","MK","BA","JE","IM","FO","GB","BB","LI","MC","GG","XK","NO","IE","LU","NE","EE","LT","NL","BE","DK","BY","LV","MT","GR","CY","SI","SK","HR","BG","CZ","HU","AT","PT")), aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label *100, 1)), "%")), size = 3, color = "black", check_overlap = TRUE,fontface = "bold",lineheight = 0.75) +
  geom_text(data = filter(EU_POP_CENTROIDS, geo %in% c("BG","CZ","HU","AT")), aes(x = st_coordinates(geometry)[,1], y = st_coordinates(geometry)[,2], label = paste0(geo, "\n", ifelse(label >= 0, " ", ""), sprintf("%.1f", round(label *100, 1)), "%")), size = 2.5, color = "black", check_overlap = TRUE,fontface = "bold",lineheight = 0.75) +
...
ggsave(dpi = "retina",plot = EU_POP_GRAPH, "EU POP Graph.png", type = "cairo-png", width = 9.02, height = 5.76, units = "in") #cairo gets rid of anti aliasing
