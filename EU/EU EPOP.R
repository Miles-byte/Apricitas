...
EU_EPOP_LINE_GRAPH <- ggplot() + #plotting car manufacturing
  geom_line(data=EU_EPOP_LINE, aes(x=time,y= EA20/100,color="Euro Area"), size = 1.25) +
  geom_line(data=EU_EPOP_LINE, aes(x=time,y= EU27_2020/100,color="European Union"), size = 1.25) +
  xlab(NULL) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), expand = c(0,0), limits = c(0.75,.83)) +
  ylab(NULL) +
  ggtitle(NULL) +
...
EU_EPOP_MAP_SMALL <- ggplot(data = EU_EPOP_SHP, aes(fill = values)) +
  geom_sf(color = NA) +
  geom_sf(color = "black", fill = NA, lwd = 0.35) + # Black borders for states
  scale_fill_gradientn(colors = c("#EE6055","#F5B041","#FFE98F", "#AED581","#00A99D","#3083DC"), label = scales::number_format(suffix = "%", accuracy = 1), expand = c(0,0)) +
  #ggtitle("EU Year-on-Year Real GDP Growth: Q2 2025") +
...
tgrob <- text_grob(paste0("  EU 25-54 Employment Rate, %: ", "Q", quarter(EU_EPOP_SHP$time[10]), " ", year(EU_EPOP_SHP$time[10])),face = "bold",size = 29,color = "white")
...
EU_EPOP_GGARRANGE <- ggarrange(EU_EPOP_LINE_GRAPH,EU_EPOP_MAP_SMALL, ncol = 2, nrow = 1, heights = c(5,20), widths = 10, common.legend = FALSE) + bgcolor("#252A32") + border("#252A32")

EU_EPOP_GGARRANGE <- ggarrange(plot_0,EU_EPOP_GGARRANGE, nrow = 2, heights = c(4,20), widths = 10) %>%
...
ggsave(dpi = "retina",plot = EU_EPOP_GGARRANGE, "EU EPOP GGARANGE Graph.png", type = "cairo-png", width = 9.02, height = 5.76, units = "in")
