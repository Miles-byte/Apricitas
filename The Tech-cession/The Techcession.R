...
QSS_Data_Graph <- ggplot() + #plotting net tightening data
  ...
  ggtitle("The Great Tech Slowdown") +
  labs(caption = "Graph created by @JosephPolitano using Census Bureau data", subtitle = "Revenue Growth Slowed Down Across Tech Sectors in 2022 after Booming During Early COVID") +
  ...
  annotation_custom(apricitas_logo_rast, xmin = as.Date("2018-01-01")-(.1861*(today()-as.Date("2018-01-01"))), xmax = as.Date("2018-01-01")-(0.049*(today()-as.Date("2018-01-01"))), ymin = 0-(.3*.45), ymax = 0) + #these repeated sections place the logo in the bottom-right of each graph. The first number in all equations is the chart's origin point, and the second number is the exact length of the x or y axis
  coord_cartesian(clip = "off")
