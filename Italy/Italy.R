...
EMPLOY_GROWTH_IND_ITA_graph <- ggplot(data = EMPLOYMENT_INDEXED_NACE_2_ITALY, aes(x = date, y = value/1000, fill = name)) + #plotting permanent and temporary job losers
  ...
  scale_y_continuous(labels = scales::number_format(accuracy = 0.5, suffix = "M"), expand = c(0,0), limits = c(-2,round(max(EMPLOYMENT_INDEXED_NACE_2_ITALY_LINE_MAX$value / 1000), 1))+.1) +
  ...
  annotation_custom(apricitas_logo_rast, xmin = as.Date("2019-10-01")-(.1861*(today()-as.Date("2019-10-01"))), xmax = as.Date("2019-10-01")-(0.049*(today()-as.Date("2019-10-01"))), ymin = -2-(.3*(round(max(EMPLOYMENT_INDEXED_NACE_2_ITALY_LINE_MAX$value / 1000), 1)+1.6)), ymax = -2) + #these repeated sections place the logo in the bottom-right of each graph. The first number in all equations is the chart's origin point, and the second number is the exact length of the x or y axis
  coord_cartesian(clip = "off")
