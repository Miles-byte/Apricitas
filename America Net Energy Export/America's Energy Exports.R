...
US_PRIMARY_ENERGY_NET_EXPORTS_Graph <- ggplot() + #plotting nat gas exports
  ...
  geom_line(data = US_TOTAL_EXPORTS, aes(x = date, y = value, color = "Total Primary Energy"), size = 1.25) +
  xlab("Date") +
  ...
  ggtitle("America's Rise as an Energy Exporter") +
  ...
  scale_color_manual(name= "Primary Energy Net Exports\n(Quadrillion BTUs, Annual)",values = c("#FFE98F","#00A99D","#EE6055","#A7ACD9","#9A348E","#3083DC","RED"), breaks = c("Total Primary Energy","Crude Oil and Petroleum Products","Natural Gas")) +
...
FINISHED_MOTOR_GASOLINE_SUPPLIED_Graph <- ggplot() + #plotting US Crude Production
  ...
  xlab("Date") +
  scale_y_continuous(labels = scales::number_format(suffix = " MMbbl", accuracy = 1), limits = c(5.75,10),breaks = c(6,7,8,9,10), expand = c(0,0)) +
  ylab("Mbbl Per Day") +
  ggtitle("America's Oil Recovery") +
  labs(caption = "Graph created by @JosephPolitano using EIA data",subtitle = "US Oil Production Is Closing in on Record High Levels") +
